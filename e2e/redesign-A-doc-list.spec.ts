...
test.describe('Redesign-A — DocumentListPanel', () => {
  test('TC-RADL-01 — documents page loads and shows document list', async ({ page }) => {
    await page.goto('/documents')
    await expect(page.locator('[data-testid="upload-btn"]')).toBeVisible()
  })

  test('TC-RADL-02 — filter search input is visible on documents page', async ({ page }) => {
    await page.goto('/documents')
  ...
  test.skip('TC-RADL-03 — document-list-panel testid visible after Phase 54 integration', async ({ page }) => {
    await page.goto('/documents')
  ...
  test.skip('TC-RADL-04 — filter bar narrows results in DocumentListPanel', async ({ page }) => {
    await page.goto('/documents')
