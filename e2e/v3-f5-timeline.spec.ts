...
test.describe('V3-F5 — Timeline Activity Date', () => {
  test('TC-F5-01 — timeline entry date is the service date extracted from PDF body', async ({ page }) => {
  ...
  test('TC-F5-02 — timeline entry falls back to filename date when PDF body has no service date', async ({ page }) => {
    await page.goto('/documents')
    await page.getByRole('button', { name: /upload/i }).click()
  ...
  test('TC-F5-03 — timeline entry falls back to upload date when neither PDF body nor filename has a date', async ({ page }) => {
  ...
  test('TC-F5-04 — timeline entry description format is "{date} {SPECIALTY} with {Title} {Provider}"', async ({ page }) => {
    await page.goto('/documents')
    await page.getByRole('button', { name: /upload/i }).click()
    await page.locator('input[type="file"]').setInputFiles(PHYSIO_MOCK_PDF)
    await page.waitForSelector('[data-testid="upload-review-step"]')
  ...
  test('TC-F5-05 — user can override the extracted activity date before saving', async ({ page }) => {
    await page.goto('/documents')
    await page.getByRole('button', { name: /upload/i }).click()
    await page.locator('input[type="file"]').setInputFiles(PHYSIO_MOCK_PDF)
    await page.waitForSelector('[data-testid="upload-review-step"]')
