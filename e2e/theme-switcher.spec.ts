...
test.describe('Phase 103 — Theme Switcher', () => {
  test('TC-THEME-01 — clicking Coffee swatch applies theme-coffee class to <html>', async ({ page }) => {
  ...
  test('TC-THEME-02 — theme persists across page reload', async ({ page }) => {
    await page.goto('/settings')
    await page.waitForSelector('[data-testid="theme-picker"]')
  ...
  test('TC-THEME-03 — active swatch has aria-pressed=true', async ({ page }) => {
    await page.goto('/settings')
    await page.waitForSelector('[data-testid="theme-picker"]')
