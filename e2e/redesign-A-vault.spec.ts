...
test.describe('Redesign-A — Dark Vault Layout', () => {
  ...
  test('TC-RAVAULT-01 — icon rail renders with correct data-testid', async ({ page }) => {
    await page.goto('/documents')
    await expect(page.getByTestId('nav-rail')).toBeVisible()
  })

  test('TC-RAVAULT-02 — icon rail has 52px width', async ({ page }) => {
    await page.goto('/documents')
    const box = await page.getByTestId('nav-rail').boundingBox()
    expect(box?.width).toBe(52)
  })

  test('TC-RAVAULT-03 — all nav rail buttons have aria-label', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-04 — active nav item has amber left border', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-05 — tooltip appears on rail button hover after 400ms', async ({ page }) => {
    await page.goto('/documents')
    await page.getByTestId('nav-rail').getByRole('button').first().hover()
    await expect(page.locator('[role="tooltip"]')).toBeVisible()
  })

  test('TC-RAVAULT-06 — clicking nav rail item navigates to correct route', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-07 — document list panel renders', async ({ page }) => {
    await page.goto('/documents')
    await expect(page.getByTestId('document-list-panel')).toBeVisible()
  })

  test('TC-RAVAULT-08 — document list panel is approximately 300px wide', async ({ page }) => {
    await page.goto('/documents')
    const box = await page.getByTestId('document-list-panel').boundingBox()
    expect(box?.width).toBeCloseTo(300, -1)
  })

  test('TC-RAVAULT-09 — document rows show title, date and category pill', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-10 — search input filters document list', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-11 — clicking a document row selects it with amber left border', async ({ page }) => {
    await page.goto('/documents')
    const row = page.getByTestId('document-list-panel').locator('[data-testid="doc-row"]').first()
    await row.click()
  ...
  test('TC-RAVAULT-12 — flagged document row shows amber flag indicator', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-13 — PDF preview panel renders after selecting a document', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-14 — PDF preview toolbar shows zoom and page controls', async ({ page }) => {
    await page.goto('/documents')
    await page.getByTestId('document-list-panel').locator('[data-testid="doc-row"]').first().click()
  ...
  test('TC-RAVAULT-15 — non-PDF document shows text preview fallback', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-16 — AI insights panel renders after selecting a document', async ({ page }) => {
    await page.goto('/documents')
    await page.getByTestId('document-list-panel').locator('[data-testid="doc-row"]').first().click()
    await expect(page.getByTestId('ai-insights-panel')).toBeVisible()
  })

  test('TC-RAVAULT-17 — AI insights panel shows extracted entities section', async ({ page }) => {
    await page.goto('/documents')
    await page.getByTestId('document-list-panel').locator('[data-testid="doc-row"]').first().click()
    await expect(page.getByTestId('ai-insights-panel').getByTestId('entities-section')).toBeVisible()
  })

  test('TC-RAVAULT-18 — flagged lab values show status pills LOW / HIGH / BORDERLINE', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-19 — AI insights panel collapses when chevron clicked', async ({ page }) => {
    await page.goto('/documents')
    await page.getByTestId('document-list-panel').locator('[data-testid="doc-row"]').first().click()
  ...
  test('TC-RAVAULT-20 — AI insights panel re-expands after collapse', async ({ page }) => {
    await page.goto('/documents')
    await page.getByTestId('document-list-panel').locator('[data-testid="doc-row"]').first().click()
    const panel = page.getByTestId('ai-insights-panel')
    await panel.getByRole('button', { name: /collapse/i }).click()
  ...
  test('TC-RAVAULT-21 — vault theme CSS variable --color-bg is #0D1117', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-22 — AI insights panel auto-collapses below 1400px viewport', async ({ page }) => {
  ...
  test('TC-RAVAULT-25 — 1440px: all 4 panels visible, no expand button', async ({ page }) => {
  ...
  test('TC-RAVAULT-26 — 1280px: AI panel collapsed, expand button visible', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 900 })
    await page.goto('/documents')
    await expect(page.getByTestId('ai-panel-expand-btn')).toBeVisible()
  })

  test('TC-RAVAULT-27 — clicking expand button restores AI panel', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 900 })
    await page.goto('/documents')
    await page.getByTestId('ai-panel-expand-btn').click()
    await expect(page.getByTestId('ai-panel-expand-btn')).not.toBeVisible()
  ...
  test('TC-RAVAULT-23 — keyboard Tab reaches first rail button', async ({ page }) => {
    await page.goto('/documents')
  ...
  test('TC-RAVAULT-24 — tooltip has role=tooltip and aria-describedby wired', async ({ page }) => {
    await page.goto('/documents')
    await page.getByTestId('nav-rail').getByRole('button').first().hover()
